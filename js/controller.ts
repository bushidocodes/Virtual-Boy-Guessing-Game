  ...
  enterPlayerGuess() {
    ...
    try {
      ...
      if (result === "You Win!" || result.startsWith("You Lose.")) {
        ...
        document
          .querySelectorAll<HTMLInputElement>("#submit, #hint, #players-input")
          .forEach((el) => {
            el.disabled = true;
          });
      } else {
        document.getElementById("subtitle")!.textContent = game.isLower()
          ? "Guess Higher"
          : "Guess Lower";
      }
      game.pastGuesses.entries().forEach(([i, pastGuess]) => {
        document.querySelector(`#guess-list li:nth-child(${i + 1})`)!.textContent =
          String(pastGuess);
      });
    } catch (err) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("reset")!.addEventListener("click", () => {
    ...
    document.querySelectorAll<HTMLElement>("#guess-list .guess").forEach((el) => {
      el.textContent = "-";
    });
    document.querySelectorAll<HTMLInputElement>("#submit, #hint, #players-input").forEach((el) => {
      el.disabled = false;
    });
